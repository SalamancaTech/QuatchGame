...
const BinViewModal: React.FC<BinViewModalProps> = ({ cards, onClose, difficulty, title = "Cards in the Bin" }) => {
  return (
    ...
    >
      <div 
        className="bg-green-900/90 border-2 border-yellow-500/50 rounded-xl shadow-2xl p-4 w-11/12 max-w-md max-h-[80vh] flex flex-col animate-prompt-fade-in"
        onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside the modal
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-yellow-300">{title} ({cards.length})</h2>
          <button 
            onClick={onClose}
